<template>
  <SettingsSection>
    <div class="grid grid-cols-1 gap-4 m-4">
      <SettingsFieldGroup
        v-for="group in pianoGroups"
        :key="group.titleKey"
        :fields="group.fields"
        :model="model"
        :title-key="group.titleKey"
        :icon="group.icon"
        i18n-prefix="settings.pianoSettings"
        @update="update"
      />
    </div>
  </SettingsSection>
...
import { pianoGroups } from "./schema";

const settingsStore = useSettingsStore();

const model = computed(
  () => settingsStore.settings.piano as unknown as SettingsFieldModel,
);

function update(key: string, value: boolean | number | string | null) {
  settingsStore.updateSetting(`piano.${key}`, value);
}
</script>
